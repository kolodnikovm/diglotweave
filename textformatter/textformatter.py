...
FILE_NAME = os.path.join(SCRIPT_PATH, 'text.txt')
VOCAB = {
    "see": "видеть",
    "it": "это",
    "think": "думать"
...
class TextFormatter:
    ...
    def save_to_csv(self):
        ...
        with open(SCRIPT_PATH + '\words_to_swap.csv', 'w+', newline='', encoding='utf-8') as result_dict:
            print(result_dict.name)
            writer = csv.writer(result_dict)
            words_to_swap = self.find_words_by_translate()
            for eng_word in words_to_swap.keys():
                for word in words_to_swap[eng_word]:
                    writer.writerow([eng_word, word])
